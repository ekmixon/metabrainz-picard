...
class File(QtCore.QObject, Item):
    ...
    def _loading_finished(self, callback, result=None, error=None):
        ...
        if not config.setting["ignore_existing_acoustid_fingerprints"]:
            if fingerprints := self.metadata.getall('acoustid_fingerprint'):
    ...
    def _guess_tracknumber_and_title(self, metadata):
        if missing := {'tracknumber', 'title'} - set(metadata):
    ...
    def _save_images(self, dirname, metadata):
        ...
        if config.setting["save_only_one_front_image"]:
            if front := metadata.images.get_front_image():
                images.append(front)
        if not images:
            images = metadata.images
        for image in images:
    ...
    def _info(self, metadata, file):
        ...
        metadata['~format'] = self.NAME or self.__class__.__name__.replace('File', '')
